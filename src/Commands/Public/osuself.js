...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('rank_self')
        .setDescription('Muestra tus estadísticas de osu!.'),
    async execute(interaction) {
        const user = await OsuUser.findOne({ discordId: interaction.user.id });
        if (!user) {
            return interaction.reply({
                content: 'No has sincronizado tu cuenta de osu!. Usa `/create` para sincronizarla.',
                ephemeral: true,
            });
        }
        ...
        const fetchStats = async mode => {
            try {
                const osuUser = await osuApi.getUser({ u: user.osuId, m: mode });
                ...
                return new EmbedBuilder()
                    .setTitle('Error')
                    .setDescription('Hubo un problema al obtener las estadísticas. Por favor, inténtalo nuevamente.')
                    .setColor('#FF0000');
            }
        };
        ...
        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder()
                ...
                .setEmoji('🎵')
                .setDisabled(currentMode === 0), // Disable button if already selected
            new ButtonBuilder()
                .setCustomId('taiko')
                .setLabel('osu!taiko')
                .setStyle(ButtonStyle.Primary)
